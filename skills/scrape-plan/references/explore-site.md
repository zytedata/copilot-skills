...
Expect minutes per call. It prints `Crawled N pages` to stderr every 15 seconds; half-filled page directories and log entries mid-run are normal. Give the call a timeout that covers the whole run, so its output comes back from that one call. NEVER interrupt it, and NEVER start a second call while one is running.
